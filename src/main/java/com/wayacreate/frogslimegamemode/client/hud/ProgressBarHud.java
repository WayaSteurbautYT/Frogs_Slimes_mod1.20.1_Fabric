...
public class ProgressBarHud {
    ...
    public static void render(GuiGraphics context, Minecraft client) {
        ...
        int textY = y - 12;
        context.drawTextWithShadow(client.textRenderer, Component.literal(percentage), textX, textY, 0xFFFFFF);
        
        // Draw label
        String label = "Journey Progress";
        int labelX = x + BAR_WIDTH / 2 - client.textRenderer.getWidth(label) / 2;
        int labelY = y + BAR_HEIGHT + 5;
        context.drawTextWithShadow(client.textRenderer, Component.literal(label), labelX, labelY, 0xAAAAAA);
    }
